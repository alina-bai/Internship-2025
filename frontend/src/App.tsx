...
import ChatSidebar from "./components/ChatSidebar"; // ⭐ новый импорт
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        setErrorMessage(
          "Сессия истекла или токен недействителен. Пожалуйста, выполните повторный вход."
        );
        setTimeout(() => onLogout(), 2000);
      } else {
        setErrorMessage(
          "Ошибка связи с сервером. Проверьте ваш API-ключ Gemini и логи бэкенда."
        );
      }
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (!isSignUp && response.data.token) {
        ...
        try {
          ...
          console.error(
            "Auto-login error after successful registration:",
            loginError
          );
          setIsSignUp(false);
          setMessage("error");
      ...
      <div className="form-container">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {message === "error" && (
            <p className="error">
              {isSignUp
                ? "Регистрация не удалась. Ошибка сервера (500) или сети."
                : "Неверный Email или пароль"}
            </p>
          )}
          {message === "error_bad_request" && (
            <p className="error">
              Регистрация не удалась (400). Проверьте требования к Email и
              паролю.
            </p>
          )}
          {message === "error_exists" && (
            <p className="error">
              Пользователь с таким Email уже существует. Войдите или используйте
              другой.
            </p>
          )}
          {message === "success" && (
            <p className="success">
              {isSignUp
                ? "Аккаунт успешно создан! Автоматический вход..."
...
const MainApp = () => {
  ...
  // ⭐ новое состояние: выбранный чат (для подсветки в sidebar)
  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          token && userEmail ? (
            // ⭐ здесь вставляем layout: слева sidebar, справа чат
